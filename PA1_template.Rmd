...
d[grepl(pattern = "segunda-feira|terça-feira|quarta-feira|quinta-feira|sexta-feira", x = `Day of Week`), "weekday or weekend"] <- "weekday"
d[grepl(pattern = "sábado|domingo", x = `Day of Week`), "weekday or weekend"] <- "weekend"
...
```{r}
d[is.na(steps), "steps"] <- d[, c(lapply(.SD, median, na.rm = TRUE)), .SDcols = c("steps")]
int <- d[, c(lapply(.SD, mean, na.rm = TRUE)), .SDcols = c("steps"), by = .(interval, 'weekday or weekend')] 
ggplot(int , aes(x = interval , y = steps, color='weekday or weekend')) + geom_line() + facet_wrap(~'weekday or weekend' , ncol = 1, nrow=2)
```
